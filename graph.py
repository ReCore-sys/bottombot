...
def create(_data: list) -> Image:
    ...
    if len(_data) == 1:
        ...
        draw.text((5, 35), "$" + str(round(_data[0][1], 2)),
                  font=fontloader(25), fill=(255, 255, 255))
    else:
        ...
        for i, x in enumerate(points):
            ...
            graphutils.draw_line_antialiased(
                draw, _img, i*x_spacing+25, 275-(x*y_spacing), last[0], last[1], color(times[i]))
            # Save the dot's position as the last point
            last = (i*x_spacing+25, 275-(x*y_spacing))
        draw.line(((25, 275-(avg*y_spacing)), (975, 275-(avg*y_spacing))),
                  fill=(255, 255, 255, 100))
        # Write the max and min values on the left hand side
        draw.text((0, 280), "$" + str(round(y_min, 1)),
                  font=fontloader(10), fill=(255, 255, 255))
        draw.text((0, Y_max-15), "$" + str(round(point_max, 1)),
                  font=fontloader(10), fill=(255, 255, 255))
        ...
        draw.text((50, 10), text,
                  font=fontloader(16), fill=(255, 255, 255))
